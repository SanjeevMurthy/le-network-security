...
Requests first pass through **AWS WAF and Shield**, providing protection against DDoS attacks and common web exploits. Lightweight logic at the edge using **CloudFront Functions** ensures request normalization before forwarding.

For protected content, a **Lambda@Edge layer validates signed URLs or JWTs**, ensuring only authorized users can access video streams. Signed URLs are generated by an **Auth Service** using CloudFront key pairs with expiration policies.

Content delivery follows two paths:
- **Cache HIT** → Served directly from CloudFront (low latency)
- **Cache MISS** → Routed to origins:
  - **S3 (via Origin Access Control)** for static assets and video segments (secure, no public access)
  - **ALB** for dynamic content like APIs and video manifests (generated by video packager)

The **content ingestion pipeline** handles media processing:
- Videos are uploaded to S3 (raw bucket)
- Triggered Lambda initiates **AWS MediaConvert** to transcode into multiple bitrates (adaptive streaming)
- Processed outputs are stored in S3 for CDN delivery

To prevent origin bypass:
- CloudFront adds a **custom secret header**
- ALB validates this header and rejects unauthorized requests
- S3 access is restricted via **OAC (SigV4 signed requests)**

Finally, **observability** is built in:
- Access logs stored in S3 and queryable via Athena
- Real-time logs streamed via Kinesis
- Metrics like cache hit ratio and latency tracked in CloudWatch

Overall, this design ensures **high performance (edge caching), security (WAF, signed URLs, origin protection), and scalability (CDN + S3 + stateless services)**, making it ideal for large-scale video streaming platforms.
